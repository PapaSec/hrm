...
class Index extends Component
{
    ...
    public function genetatePayroll()
    {
        $this->validate();
        $date = Carbon::parse($this->monthYear);
        if (Payroll::inCompany()->where('month', $date->format('m'))->where('year', $date->format('Y'))->exists()) {
            throw ValidationException::withMessages(['monthYear' => 'Paroll already generated for this month']);
        } else {
            $payroll = new Payroll();
            ...
            foreach (Employee::inCompany()->get() as $employee) {
                $contract = $employee->getActiveContract($date->startOfMonth()->toDateString());
                if ($contract) {
